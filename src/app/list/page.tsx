...
export default function Component() {
  ...
  const [photos, setPhotos] = useState<{ id: string; src: string; alt: string }[]>([])
  ...
  const handlePhotoUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      const newPhoto = {
        id: `photo-${Date.now()}`,
        src: fileURL,
        alt: `Photo for ${selectedActivity.name}`
      }
  ...
  return (
      <div className={`flex flex-col h-screen ${isDarkMode ? "bg-[#1c1c23] text-white" : "bg-gray-100 text-gray-900"} font-sans`}>
        <header className={`flex justify-between items-center p-4 ${isDarkMode ? "bg-[#2c2c35]" : "bg-white"} shadow-md`}>
          <h1 className={`text-2xl font-bold ${isDarkMode ? "text-[#7a7bff]" : "text-blue-600"}`}>
            Lyf Circle
          </h1>
        ...
        <main className="flex-1 overflow-y-auto p-4 flex flex-col">
          <div className={`transition-all duration-300 ease-in-out ${isTransitioning ? "opacity-0 transform translate-x-full" : "opacity-100 transform translate-x-0"}`}>
            {selectedActivity ? (
                <div className={`${isDarkMode ? "bg-[#2c2c35]" : "bg-white"} rounded-lg shadow-lg p-6 flex flex-col flex-grow`}>
                  <button
                ...
                <span className="text-5xl mr-6" role="img" aria-label={selectedActivity.name}>
                    ...
                    <div>
                      ...
                      <div className="flex items-start flex-col text-sm">
                        <div className={`flex items-center ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>
                          ...
                          <span>
                        {selectedActivity.date.month}/{selectedActivity.date.day} {selectedActivity.date.weekday} {selectedActivity.date.time}
                      </span>
                        </div>
                        <div className={`flex items-center ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>
                  ...
                  <p className={`mb-6 ${isDarkMode ? "text-gray-300" : "text-gray-700"} text-lg leading-relaxed`}>
                    {selectedActivity.description}
                  </p>

                  {selectedActivity.icebreaker && (
                      <div  className={`mb-6 ${isDarkMode ? "bg-[#4a3f8f] text-white" : "bg-blue-100 text-blue-900"} rounded-lg p-6 shadow-lg transform transition-transform duration-200`}>
                  ...
                  <div className="space-y-6 flex flex-col flex-grow">
                    <div className="mt-6 flex-grow flex justify-center">
                      {attend.length > 0 && selectedActivity && attend.find((a) => a.id === selectedActivity.id)?.attend === false ? (
                          <button
                          ...
                          selectedActivity && (
                              <div className="flex flex-col w-full">
                                <ChatComponent
                                    ...
                                    initialMessages={[
                                      {
                                        content: "Welcome, new participant! Please say hello to new friend!",
                                        sender: "host",
                                        senderId: "ai",
                                        type: "CHAT",
                    ...
                    {attend.length > 0 && selectedActivity && attend.find((a) => a.id === selectedActivity.id)?.attend === true && (
                        <div className="mt-8">
                          ...
                          <div className="grid grid-cols-3 gap-2">
                            ...
                            <div
                                className="aspect-square border-2 border-dashed border-gray-300 rounded-lg flex items-center justify-center cursor-pointer hover:border-gray-400 transition-colors duration-300"
